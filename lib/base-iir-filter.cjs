...
class BaseIIRFilter {
  ...
  static realize(kind, cutoffHz, fs, proto) {
    ...
    if (kind === 'lowpass' || kind === 'highpass') {
      w1 = utils.Util.prewarp(/** @type {number} */(cutoffHz), fs);
    } else {
      const [f1, f2] = /** @type {[number,number]} */(cutoffHz);
      w1 = utils.Util.prewarp(f1, fs);
      w2 = utils.Util.prewarp(f2, fs);
      B  = w2 - w1;
      w0 = Math.sqrt(w1 * w2);
    }
    const mapHP = (s) => complex.ComplexNum.div(complex.ComplexNum.of(w1, 0), s);
    const mapBP = (s) => {
      ...
      if (Math.abs(s.re) < 1e-14 && Math.abs(s.im) < 1e-14) {
        ...
        return [complex.ComplexNum.of(0, w0), complex.ComplexNum.of(0, -w0)];
      }
      
      // For bandpass: s_new = (B * s_lp ± sqrt((B * s_lp)^2 - 4 * w0^2)) / 2
      const Bs = complex.ComplexNum.scale(s, B);
      const w02 = complex.ComplexNum.of(w0 * w0, 0);
      
      // Calculate discriminant: (B * s_lp)^2 - 4 * w0^2
      const Bs2 = complex.ComplexNum.mul(Bs, Bs);
      const fourW02 = complex.ComplexNum.scale(w02, 4);
      const discriminant = complex.ComplexNum.sub(Bs2, fourW02);
      const sqrtDisc = ComplexSqrt(discriminant);
      
      // Two solutions
      const s1 = complex.ComplexNum.scale(complex.ComplexNum.add(Bs, sqrtDisc), 0.5);
      const s2 = complex.ComplexNum.scale(complex.ComplexNum.sub(Bs, sqrtDisc), 0.5);
    ...
    const mapBS = (s) => {
      ...
      if (Math.abs(s.re) < 1e-14 && Math.abs(s.im) < 1e-14) {
        // Handle s=0 case
        return [complex.ComplexNum.of(0, 0), complex.ComplexNum.of(0, 0)];
      }
      
      // For bandstop: solve s_lp = (B * s) / (s^2 + w0^2)
      // This gives: s_lp * s^2 - B * s + s_lp * w0^2 = 0
      const Bs = complex.ComplexNum.scale(s, B);
      const sW02 = complex.ComplexNum.mul(s, complex.ComplexNum.of(w0 * w0, 0));
      
      // Discriminant: B^2 - 4 * s_lp^2 * w0^2
      const B2 = complex.ComplexNum.mul(Bs, Bs);
      const fourSW02 = complex.ComplexNum.scale(sW02, 4);
      const discriminant = complex.ComplexNum.sub(B2, fourSW02);
      const sqrtDisc = ComplexSqrt(discriminant);
      
      // Two solutions
      const denom = complex.ComplexNum.scale(s, 2);
      const s1 = complex.ComplexNum.div(complex.ComplexNum.add(Bs, sqrtDisc), denom);
      const s2 = complex.ComplexNum.div(complex.ComplexNum.sub(Bs, sqrtDisc), denom);
    ...
    if (kind === 'lowpass') {
      for (const s of proto.z) push(zA, complex.ComplexNum.scale(s, w1));
      for (const p of proto.p) push(pA, complex.ComplexNum.scale(p, w1));
      ...
      for (let i = 0; i < extraZeros; i++) zA.push(complex.ComplexNum.of(0, 0));
      ...
      for (let i = 0; i < orderDiff; i++) {
        zA.push(complex.ComplexNum.of(0, 0));
      }
      ...
      for (let i = 0; i < orderDiff; i += 2) {
        zA.push(complex.ComplexNum.of(0,  w0));
        zA.push(complex.ComplexNum.of(0, -w0));
      }
    ...
    const { z, p, k } = utils.Util.bilinearZPK(zA, pA, proto.k || 1, fs);
    let { b, a } = utils.Util.zpk2tf(z, p, k);
...
exports.default = BaseIIRFilter;
//# sourceMappingURL=base-iir-filter.cjs.map
