...
class Filter {
    ...
    frequencyResponse(fs, N = 1024) {
        const fr = zdomain.ZDomain.freqz(this.b, this.a, N);
    ...
    static designFIR(kind, cutoffHz, fs, order, window='hann'){
        const tf = fir.FIRDesigner.design(kind, cutoffHz, fs, order, window);
    ...
    static designElliptic(kind, cutoffHz, fs, order, rp = 1, rs = 40) {
        const result = iir.IIRDesigner.elliptic(kind, cutoffHz, fs, order, rp, rs);
        return new Filter(result.b, result.a, result.sections);
    }

    static designBessel(kind, cutoffHz, fs, order) {
        const result = iir.IIRDesigner.bessel(kind, cutoffHz, fs, order);
        return new Filter(result.b, result.a, result.sections);
    }
}
