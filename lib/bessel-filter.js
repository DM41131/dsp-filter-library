...
class BesselFilter {
  ...
  static durandKerner(coeffs) {
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
          if (i !== j) {
            const diff = ComplexNum.sub(current, roots[j]);
            denominator = ComplexNum.mul(denominator, diff);
          }
        }
        
        const correction = ComplexNum.div(value, denominator);
        const newRoot = ComplexNum.sub(current, correction);
  ...
  static evaluatePolynomial(coeffs, z) {
    let result = ComplexNum.of(0, 0);
    let zPower = ComplexNum.of(1, 0);
    
    for (let i = coeffs.length - 1; i >= 0; i--) {
      result = ComplexNum.add(result, ComplexNum.scale(zPower, coeffs[i]));
      zPower = ComplexNum.mul(zPower, z);
    }
...
export { BesselFilter, BesselFilter as default };
//# sourceMappingURL=bessel-filter.js.map
