...
class BaseIIRFilter {
  ...
  static fromPrototype(kind, fs, polesNorm, normalizeAt, wc) {
    ...
    const pairs = utils.Util.pairConjugates(polesNorm);
    for (const pair of pairs) {
      ...
      sections.push(utils.Util.bilinearMapBiquad(b2, b1, b0, A2, A1, A0, fs));
    }

    // Compose polynomials, then digital normalization at z0.
    let bPoly = [1], aPoly = [1];
    for (const s of sections) { 
      bPoly = utils.Util.polymul(bPoly, s.b); 
      aPoly = utils.Util.polymul(aPoly, s.a); 
    }

    const g = 1 / utils.Util.evalHzAtZ(bPoly, aPoly, normalizeAt);
...
class ButterworthFilter extends BaseIIRFilter {
  ...
  static calculatePoles(order) {
    ...
    for (let k = 0; k < order; k++) {
      ...
      const p = complex.ComplexNum.of(Math.cos(theta), Math.sin(theta));
  ...
  static designLowHighPass(kind, cutoffHz, fs, order) {
    const fc = /** @type {number} */ (cutoffHz);
    const wc = utils.Util.prewarp(fc, fs);
  ...
  static designBandPass(cutoffHz, fs, order) {
    ...
    const combinedB = utils.Util.polymul(hpFilter.b, lpFilter.b);
    const combinedA = utils.Util.polymul(hpFilter.a, lpFilter.a);
  ...
  static designBandStop(cutoffHz, fs, order) {
    ...
    const commonA = utils.Util.polymul(lpFilter.a, hpFilter.a);
    const lpNum = utils.Util.polymul(lpFilter.b, hpFilter.a);
    const hpNum = utils.Util.polymul(hpFilter.b, lpFilter.a);
    const combinedB = utils.Util.polyadd(lpNum, hpNum);
...
class ChebyshevFilter extends BaseIIRFilter {
  ...
  static calculatePoles(order, ripple) {
    ...
    for (let k = 0; k < order; k++) {
      ...
      const p = complex.ComplexNum.of(re, im);
      if (p.re < 0) poles.push(p);
    }
    return poles;
  }
...
class ChebyshevType2Filter extends BaseIIRFilter {
  ...
  static calculatePolesAndZeros(order, rs) {
    ...
    for (let k = 1; k <= order; k++) {
      ...
      poles.push(complex.ComplexNum.of(re, im));
    }
  ...
  static mapSectionLP_HP(kind, polePair, zeroW, wc, fs) {
    ...
    return utils.Util.bilinearMapBiquad(B2, B1, B0, A2, A1, A0, fs);
  }
  ...
  static designLPHP(kind, cutoffHz, fs, order, rs) {
    const wc = utils.Util.prewarp(cutoffHz, fs);
    const { poles, zerosW } = this.calculatePolesAndZeros(order, rs);

    // Pair poles for SOS
    const polePairs = utils.Util.pairConjugates(poles);
    ...
    let b = [1], a = [1];
    for (const s of sections) {
      b = utils.Util.polymul(b, s.b);
      a = utils.Util.polymul(a, s.a);
    }

    // Normalize digital gain
    const zNorm = (kind === 'lowpass') ? 1 : -1; // DC for LP, Nyquist for HP
    const g = 1 / utils.Util.evalHzAtZ(b, a, zNorm);
  ...
  static designBandPass(cutoffHz, fs, order, stopbandAttenuation) {
    ...
    return {
      b: utils.Util.polymul(hp.b, lp.b),
      a: utils.Util.polymul(hp.a, lp.a),
  ...
  static designBandStop(cutoffHz, fs, order, stopbandAttenuation) {
    ...
    const a = utils.Util.polymul(lp.a, hp.a);
    const b_lp = utils.Util.polymul(lp.b, hp.a);
    const b_hp = utils.Util.polymul(hp.b, lp.a);
    const b = utils.Util.polyadd(b_lp, b_hp);
...
class LinkwitzRileyFilter extends BaseIIRFilter {
  ...
  static design(kind, cutoffHz, fs, order) {
    ...
    const cascadedB = utils.Util.polymul(baseFilter.b, baseFilter.b);
    const cascadedA = utils.Util.polymul(baseFilter.a, baseFilter.a);
...
class EllipticFilter extends BaseIIRFilter {
    ...
    static designHighPass(cutoffHz, fs, order, passbandRipple, stopbandAttenuation) {
        ...
        const transformedPoles = poles.map(p => complex.C.div(complex.C.of(1, 0), p));
    ...
    static calculateEllipticPolesAndZeros(order, passbandRipple, stopbandAttenuation) {
        ...
        for (let i = 1; i <= Math.floor(order / 2); i++) {
            ...
            poles.push(complex.C.of(real, imag));
            poles.push(complex.C.of(real, -imag)); // Conjugate
            
            // Calculate zero location
            const zeroReal = 1 / (k * sn);
            const zeroImag = 0;
            zeros.push(complex.C.of(zeroReal, zeroImag));
            zeros.push(complex.C.of(zeroReal, -zeroImag)); // Conjugate
        }
        ...
        if (order % 2 === 1) {
            const u = K / order;
            const sn = this.jacobiSn(u, k);
            poles.push(complex.C.of(-sn, 0));
            zeros.push(complex.C.of(1 / (k * sn), 0));
        }
...
class BesselFilter extends BaseIIRFilter {
    ...
    static calculateBesselPoles(order) {
        ...
        const poleSets = {
            1: [complex.C.of(-1, 0)],
            2: [complex.C.of(-1.5, 0.8660254037844386), complex.C.of(-1.5, -0.8660254037844386)],
            3: [complex.C.of(-2.322185354626086, 0), complex.C.of(-1.838907322686957, 1.754380959783721), complex.C.of(-1.838907322686957, -1.754380959783721)],
            4: [complex.C.of(-2.103789397179628, 0.6657060219931349), complex.C.of(-2.103789397179628, -0.6657060219931349), complex.C.of(-1.896210602820372, 1.744447419188405), complex.C.of(-1.896210602820372, -1.744447419188405)],
            5: [complex.C.of(-2.324674303181611, 0), complex.C.of(-2.048290428681656, 1.000044768299361), complex.C.of(-2.048290428681656, -1.000044768299361), complex.C.of(-1.673416736234146, 1.673416736234146), complex.C.of(-1.673416736234146, -1.673416736234146)],
            6: [complex.C.of(-2.13290631146253, 0.4718706301774892), complex.C.of(-2.13290631146253, -0.4718706301774892), complex.C.of(-1.905412542845325, 1.552914270615124), complex.C.of(-1.905412542845325, -1.552914270615124), complex.C.of(-1.606938159156189, 1.606938159156189), complex.C.of(-1.606938159156189, -1.606938159156189)],
            7: [complex.C.of(-2.324674303181611, 0), complex.C.of(-2.048290428681656, 1.000044768299361), complex.C.of(-2.048290428681656, -1.000044768299361), complex.C.of(-1.673416736234146, 1.673416736234146), complex.C.of(-1.673416736234146, -1.673416736234146), complex.C.of(-1.414213562373095, 1.414213562373095), complex.C.of(-1.414213562373095, -1.414213562373095)],
            8: [complex.C.of(-2.13290631146253, 0.4718706301774892), complex.C.of(-2.13290631146253, -0.4718706301774892), complex.C.of(-1.905412542845325, 1.552914270615124), complex.C.of(-1.905412542845325, -1.552914270615124), complex.C.of(-1.606938159156189, 1.606938159156189), complex.C.of(-1.606938159156189, -1.606938159156189), complex.C.of(-1.414213562373095, 1.414213562373095), complex.C.of(-1.414213562373095, -1.414213562373095)],
            9: [complex.C.of(-2.324674303181611, 0), complex.C.of(-2.048290428681656, 1.000044768299361), complex.C.of(-2.048290428681656, -1.000044768299361), complex.C.of(-1.673416736234146, 1.673416736234146), complex.C.of(-1.673416736234146, -1.673416736234146), complex.C.of(-1.414213562373095, 1.414213562373095), complex.C.of(-1.414213562373095, -1.414213562373095), complex.C.of(-1.224744871391589, 1.224744871391589), complex.C.of(-1.224744871391589, -1.224744871391589)],
            10: [complex.C.of(-2.13290631146253, 0.4718706301774892), complex.C.of(-2.13290631146253, -0.4718706301774892), complex.C.of(-1.905412542845325, 1.552914270615124), complex.C.of(-1.905412542845325, -1.552914270615124), complex.C.of(-1.606938159156189, 1.606938159156189), complex.C.of(-1.606938159156189, -1.606938159156189), complex.C.of(-1.414213562373095, 1.414213562373095), complex.C.of(-1.414213562373095, -1.414213562373095), complex.C.of(-1.224744871391589, 1.224744871391589), complex.C.of(-1.224744871391589, -1.224744871391589)]
        };
...
exports.BaseIIRFilter = BaseIIRFilter;
...
exports.IIRDesigner = IIRDesigner;
exports.LinkwitzRileyFilter = LinkwitzRileyFilter;
//# sourceMappingURL=iir-Btez-fr5.js.map
