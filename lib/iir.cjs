...
class IIRDesigner {
  ...
  static cheby2PolesAndZeros(n, rs) {
    ...
    for (let k = 1; k <= n; k++) {
      ...
      if (re < 0) poles.push(complex.ComplexNum.of(re, im));
    }
  ...
  static fromPrototype(kind, fs, polesNorm, normalizeAt, wc) {
    ...
    let bPoly = [1], aPoly = [1];
    for (const s of sections) { bPoly = utils.Util.polymul(bPoly, s.b); aPoly = utils.Util.polymul(aPoly, s.a); }
  ...
  static butterworth(kind, cutoffHz, fs, order) {
    ...
    if (kind === 'bandpass' || kind === 'bandstop') {
      ...
      const tf = fir.FIRDesigner.design(kind, [f1, f2], fs, orderFIR, 'hamming');
  ...
  static cheby2(kind, cutoffHz, fs, order, rs = 40) {
    ...
    if (kind !== 'lowpass') {
      const orderFIR = Math.max(64, order * 8);
      const tf = fir.FIRDesigner.design(kind, cutoffHz, fs, orderFIR, 'hamming');
      return { b: tf.b, a: tf.a, sections: [] };
    }
    ...
    const P = poles.map(p => complex.ComplexNum.of(p.re * ws, p.im * ws)); // analog poles
    ...
    let b = [1], a = [1];
    for (const s of sections) { b = utils.Util.polymul(b, s.b); a = utils.Util.polymul(a, s.a); }
