...
class Response {
  ...
  static magDbFromH(H){ return 20*Math.log10(Math.max(1e-16, core_Complex.Cx.abs(H))); }
  static unwrapToDeg(phRad){ return core_util.unwrapPhase(phRad).map(v=>v*180/Math.PI); }
