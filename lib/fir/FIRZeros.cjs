...
class FIRZeros {
  static fromTapsRobust(taps){
    ...
    const cand=[];
    try { for(const z of core_Roots.Roots.aberthMonic(coeffs)) if(Number.isFinite(z.re)&&Number.isFinite(z.im)) pushUnique(cand,z); } catch{/* ignore */}
    try { for(const z of core_Roots.Roots.dkScaled(coeffs))    if(Number.isFinite(z.re)&&Number.isFinite(z.im)) pushUnique(cand,z); } catch{/* ignore */}
...
function approxUnitCircleMinima(taps, want){
  ...
  for(let i=0;i<=K;i++){ const w=Math.PI*i/K; const H=digital_Response.Response.H_w_FIR(taps,w); mags[i]=Math.hypot(H.re,H.im); }
  for(let i=1;i<K;i++){
...
exports.FIRZeros = FIRZeros;
//# sourceMappingURL=FIRZeros.cjs.map
